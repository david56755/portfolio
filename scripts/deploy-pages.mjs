...
// No publicar archivos privados o inesperados que hayan llegado a dist.
execFileSync(
  process.execPath,
  [join(root, "scripts/check-publication.mjs"), "--dist"],
...
try {
  ...
  if (existing.status === 0) {
    ...
    throw new Error(
      existing.stderr || "No se pudo consultar la rama publicada.",
    );
  }
  ...
  if (git(["status", "--porcelain"], checkout)) {
    ...
    console.log(
      "Versión enviada a gh-pages. GitHub Pages finalizará la publicación.",
    );
  } else {
  ...
  if (
    dirname(checkout) !== tempRoot ||
    !basename(checkout).startsWith("portfolio-pages-")
  ) {
